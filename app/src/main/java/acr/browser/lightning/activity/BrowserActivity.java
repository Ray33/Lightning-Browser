...
//        mBrowserFrame.setLayoutTransition(new LayoutTransition());
        ...
        switch (mPreferences.getSearchChoice()) {
            case 0:
                mSearchUrl = mPreferences.getSearchUrl();
                if (!mSearchUrl.startsWith(Constants.HTTP)
                        && !mSearchUrl.startsWith(Constants.HTTPS)) {
                    mSearchUrl = Constants.GOOGLE_SEARCH;
                }
                break;
            case 1:
                mSearchUrl = Constants.GOOGLE_SEARCH;
                break;
            case 2:
                mSearchUrl = Constants.ASK_SEARCH;
                break;
            case 3:
                mSearchUrl = Constants.BING_SEARCH;
                break;
            case 4:
                mSearchUrl = Constants.YAHOO_SEARCH;
                break;
            case 5:
            ...
            case 7:
                ...
                break;
            case 8:
                mSearchUrl = Constants.DUCK_LITE_SEARCH;
                break;
            case 9:
                mSearchUrl = Constants.BAIDU_SEARCH;
                break;
            case 10:
    ...
    private void searchTheWeb(@NonNull String query) {
        ...
        String searchUrl = mSearchUrl + UrlUtils.QUERY_PLACE_HOLDER;
        query = query.trim();
        if (currentTab != null) {
    ...
    public void updateUrl(@Nullable String url, boolean shortUrl) {
        ...
        mBookmarksView.handleUpdatedUrl(url);
        final int searchEngine = mPreferences.getSearchChoice();
        //if search engine is Mobitech display only search text
